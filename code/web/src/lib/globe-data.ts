...
/**
 * Build positions from the `geo` field returned by the server-side API.
 * Falls back to hash-based position if geo is null.
 */
export function resolveNodePositionsFromApi(
  nodes: ApiNode[],
  ...
  for (const node of nodes) {
    if (node.geo) {
      ...
      result.set(node.node_id, hashNodePosition(node.node_id));
    }
  }
  return result;
}
