...
import { type NetworkStats, type ApiNode, type ApiSkill, type ApiNodeCall, getNetworkOverview, getNodeCalls } from "@/lib/api";
import {
  type GlobeNode,
  type GlobeArc,
  type ActivityEvent,
  resolveNodePositionsFromApi,
...
export default function NetworkPage() {
  ...
  useEffect(() => {
    async function load() {
      try {
        const overview = await getNetworkOverview({ nodes_limit: 100, skills_limit: 10 });

        setStats(overview.stats);
        setSkills(overview.skills);

        const nodesRes = { data: overview.nodes };

        // Resolve positions from server-side geo data (merge mock positions)
        const positions = resolveNodePositionsFromApi(nodesRes.data);
