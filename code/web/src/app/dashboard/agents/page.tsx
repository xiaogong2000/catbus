...
import { UnbindAgentDialog } from "@/components/provider/unbind-agent-dialog";
...
import { Bot, Plus, Terminal, Unlink } from "lucide-react";
...
export default function AgentsPage() {
  ...
  function handleUnbindClick(e: React.MouseEvent, agent: Agent) {
    e.preventDefault();
    e.stopPropagation();
    setUnbindTarget(agent);
  }
  ...
  return (
    <PageTransition>
      ...
      <UnbindAgentDialog
        ...
        agentName={unbindTarget?.name ?? ""}
        nodeId={unbindTarget?.node_id ?? ""}
      />
          ...
          {agents.map((agent) => (
            <Link key={agent.node_id} href={`/dashboard/agents/${agent.node_id}`}>
              <Card className="flex items-start gap-5">
                ...
                <button
                  onClick={(e) => handleUnbindClick(e, agent)}
                  className="shrink-0 self-center ml-2 p-2 rounded-lg text-text-muted hover:text-danger hover:bg-danger/10 transition-all duration-200 cursor-pointer"
                  title={t("dash.settings.unbind")}
                >
                  <Unlink size={16} />
                </button>
              </Card>
