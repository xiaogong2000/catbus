...
export async function getSkills(page = 1, limit = 50): Promise<Paginated<ApiSkill>> {
  // 优先用新接口（包含 model/ 类型），fallback 到 getNetworkOverview
  try {
    const res = await fetchApi<{ data: ApiSkill[]; total: number }>(`/network/skills?limit=${limit}`);
    return { data: res.data, total: res.total, page, limit };
  } catch {
