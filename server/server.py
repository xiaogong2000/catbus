...
def find_providers(requested_capability: str, exclude_node: str = "") -> list[tuple[Node, Capability]]:
    """
    Find online nodes that provide a given capability.

    支持模糊匹配：
      model/claude-sonnet-4   → 精确匹配
      model/claude            → 前缀匹配（所有 Claude 系列）
      model/best              → cost_tier 最高的模型
      model/cheapest          → cost_tier 最低的模型
      model/any               → 任意模型
      skill/tavily            → 精确匹配
      skill/search            → category 匹配
      echo                    → 老格式 bare name（自动加 skill/ 前缀）
    """
    if "/" not in requested_capability:
        # 老格式 bare name → 先尝试 skill/ 前缀
        requested_capability = f"skill/{requested_capability}"

    cap_type, cap_name = requested_capability.split("/", 1)

    candidates: list[tuple[Node, Capability]] = []
        ...
        for cap in node.capabilities:
            ...
            if cap_type == "model":
                if cap_name == "best":
                    ...
                    # model/claude → matches model/claude-sonnet-4
                    candidates.append((node, cap))

            # 模糊匹配 — skill/
            elif cap_type == "skill":
                # category 匹配：skill/search → category=search
                if cap.meta.get("category") == cap_name:
                    candidates.append((node, cap))

    # 排序
    if cap_type == "model":
        if cap_name == "best":
            candidates.sort(key=lambda x: COST_TIER_ORDER_DESC.get(
                x[1].meta.get("cost_tier", "free"), 5))
        elif cap_name == "cheapest":
            candidates.sort(key=lambda x: COST_TIER_ORDER_ASC.get(
                x[1].meta.get("cost_tier", "free"), 5))
