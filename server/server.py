...
class Skill:
    ...
    meta: dict = field(default_factory=dict)  # arena_elo, strengths, cost_tier
...
def find_providers(skill_name: str, exclude_node: str = "") -> list[Node]:
    ...
    When skill_name is "model/<selector>", route by capability meta.
    """
    # Check for virtual selector (model/best, model/code, etc.)
    selector = None
    ...
    # Virtual selector routing — scan all "model/*" skills across nodes
    candidates: list[tuple[Node, dict]] = []  # (node, meta)
    for node in nodes.values():
        if node.node_id == exclude_node:
            continue
        for s in node.skills:
            if not s.name.startswith("model/"):
                continue
            meta = s.meta
            if not meta:
                continue
            ...
            if selector == "best":
                candidates.append((node, {**meta, "arena_elo": elo}))
            elif selector == "cheapest":
                candidates.append((node, {**meta, "cost_tier": cost_tier}))
            elif selector == "fast":
                if "fast" in strengths:
                    candidates.append((node, {**meta, "arena_elo": elo}))
            else:
                # code/math/reasoning/vision/chinese/long
                if selector in strengths or selector == "long" and "long-context" in strengths:
                    candidates.append((node, {**meta, "arena_elo": elo}))
    ...
    if selector == "cheapest":
        min_tier = min(COST_TIER_ORDER.get(c[1].get("cost_tier", "medium"), 2) for c in candidates)
        best = [c[0] for c in candidates if COST_TIER_ORDER.get(c[1].get("cost_tier", "medium"), 2) == min_tier]
    else:
        max_elo = max(c[1].get("arena_elo", 0) for c in candidates)
        best = [c[0] for c in candidates if c[1].get("arena_elo", 0) == max_elo]
